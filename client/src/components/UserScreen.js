...
export default function UserScreen(){
    ...
    useEffect(() => {
        if(urlList.length > 0){
            ...
            setUrl(urlList[0])
            console.log('[USER SCREEN] Playlist is loaded: ', playlist)
            console.log('[USER SCREEN] Updating playlist listen count...')
            if(playlist.published)
                setCanComment(true)
            else{
                setCanComment(false)
            }
            store.updateListens(playlist, '/user')
        }
        else{
            setUrl('')
        }
    ...
    function arraysEqual(a, b) {
      ...
      function loadVideos(list){
        console.log('[USER SCREEN] Playlist clicked? ', list)
        setValue('player')
        var vidList = list.songs.map((a) => "https://www.youtube.com/watch?v=" + a.youTubeId)
        console.log('[USER SCREEN] URL List:  ', vidList)
        console.log('[USER SCREEN] URL List:  ', urlList)
        console.log('[USER SCREEN] URL list the same?', !arraysEqual(vidList, urlList))
        if(!arraysEqual(vidList, urlList)){
            console.log('[USER SCREEN] DIFFERENT URL LIST LOADED...')
            setUrlList(vidList)
            setIsPlaying(false)
            setPlaylist(list)
            if(playlist.published)
        ...
        if(vidList.length === 0){
            console.log('[USER SCREEN] NO SONGS LOADED...')
            setUrlList([])
            setPlaylist(null)
    ...
    function pauseVideo(){
        console.log('[USER SCREEN] Pausing video...')
        setIsPlaying(false)
    }

    function playVideo(){
        console.log('[USER SCREEN] Playing video...')
        setIsPlaying(true)
    }

    function goBack(){
        console.log('[USER SCREEN] Go back a song...')
        console.log('[USER SCREEN] Current Position: ', playlistPosition)
    ...
    function goForward(){
        console.log('[USER SCREEN] Go forward a song...')
        console.log('[USER SCREEN] Current Position: ', playlistPosition)
    ...
    if (store && store.idNamePairsPublished.length > 0 && search) {
        switch(sort){
            case 'Names (A-Z)':
                ...
                store.idNamePairsPublished.filter((a) => a.playlist.author.includes(search)).sort((a,b) => a.name.localeCompare(b.name, undefined , {numeric: true, sensitivity: 'base'})).reverse().map((pair) => 
                (
                    <ListCard
                        key={pair._id}
                        idNamePair={pair}
            ...
            case 'Publish Date (Newest)':
                listCard =
                // If search field is present and the sort is selected for Names (A-Z)
                store.idNamePairsPublished.filter((a) => a.playlist.author.includes(search)).sort((a,b) => a.playlist.date.localeCompare(b.playlist.date, undefined , {numeric: true, sensitivity: 'base'})).reverse().map((pair) => 
                (
                    <ListCard
                        key={pair._id}
                        idNamePair={pair}
            ...
            case 'Listens (High to Low)':
                ...
                store.idNamePairsPublished.filter((a) => a.playlist.author.includes(search)).sort((a,b) => b.playlist.listens - a.playlist.listens ).map((pair) => 
                (
                    <ListCard
                        key={pair._id}
                        idNamePair={pair}
            ...
            case 'Likes (High to Low)':
                listCard =
                // If search field is present 
                store.idNamePairsPublished.filter((a) => a.playlist.author.includes(search)).sort((a,b) => b.playlist.likes - a.playlist.likes).map((pair) => 
                (
                    <ListCard
                        key={pair._id}
                        idNamePair={pair}
            ...
            case 'Dislikes (High to Low)':
                listCard =
                // If search field is present 
                store.idNamePairsPublished.filter((a) => a.playlist.author.includes(search)).sort((a,b) => b.playlist.dislikes - a.playlist.dislikes).map((pair) => 
                (
                    <ListCard
                        key={pair._id}
                        idNamePair={pair}
            ...
            default:
                listCard =
                // If search field is present 
                store.idNamePairsPublished.filter((a) => a.playlist.author.includes(search)).map((pair) => 
                (
                    <ListCard
                        key={pair._id}
                        idNamePair={pair}
    ...
    console.log('[USER SCREEN] BACKWARD?: ', disableBackward)
    console.log('[USER SCREEN] PLAY?: ', disablePlay)
    console.log('[USER SCREEN] PAUSE?: ', disablePause)
    console.log('[USER SCREEN] FORWARD?: ', disableForward)
    ...
    let commentList = [];
    console.log('[USER SCREEN] IDNAMEPAIRS ARE: ', store.idNamePairs)
    if (store.idNamePairsPublished !== null && playlist !== null){
        ...
        console.log('[USER SCREEN] Comments are: ', commentList)
    }
    ...
    if (value === 'comments' && canComment){
        ...
        <Grid item>
            ...
            <TextField 
                ...
                onKeyDown = {(e)=> {
                    if(e.key === 'Enter'){
                        console.log('[USER SCREEN] Enter key was pressed')
                        // Update the comment section for this playlist
                        console.log('[USER SCREEN] IS THE COMMENT EMPTY?', comment === '')
                        store.updateComments(comment, playlist, '/user')
